...
# Load the trained model (Ensure you have already saved 'lasso_model.pkl')
try:
...
# List of cities and states used in training
city_list = ['Ahmedabad', 'Aizawl', 'Bangalore', 'Berhampur', 'Bhubaneswar', 'Chandigarh', 'Chennai', 'Coimbatore', 'Cuttack', 'Dehradun', 'Delhi', 'Dharmanagar', 'Dibrugarh', 'Dimapur', 'Dispur', 'Durgapur', 'Gangtok', 'Gaya', 'Ghaziabad', 'Guwahati', 'Haridwar', 'Hyderabad', 'Imphal', 'Itanagar', 'Jaipur', 'Jamshedpur', 'Jodhpur', 'Kochi', 'Kohima', 'Kolkata', 'Kozhikode', 'Lucknow', 'Madurai', 'Mangalore', 'Mumbai', 'Mysore', 'Nagpur', 'Naharlagun', 'Nainital', 'Noida', 'Patna', 'Pune', 'Puri', 'Ranchi', 'Rourkela', 'Sambalpur', 'Shillong', 'Siliguri', 'Thiruvananthapuram', 'Tura', 'Udaipur']
state_list = ['Assam', 'Bihar', 'Delhi', 'Gujarat', 'Jharkhand', 'Karnataka', 'Kerala', 'Maharashtra', 'Manipur', 'Meghalaya', 'Mizoram', 'Nagaland', 'Odisha', 'Punjab', 'Rajasthan', 'Sikkim', 'Tamil Nadu', 'Telangana', 'Tripura', 'Uttar Pradesh', 'Uttarakhand', 'West Bengal']

# Take user input
sqft = float(input("Enter the house area in sqft: "))
...
user_data = {'Area': sqft, 'BHK': bhk, 'Age': age}

# One-hot encode city and state (initialize all with 0)
for c in city_list:
    user_data[f'City_{c}'] = 1 if c == city else 0
for s in state_list:
    user_data[f'State_{s}'] = 1 if s == state else 0

# Convert user input into a DataFrame
user_df = pd.DataFrame([user_data])

# Ensure feature order matches trained model
required_features = model_lasso.feature_names_in_
user_df = user_df.reindex(columns=required_features, fill_value=0)

# Predict the house price
predicted_price = model_lasso.predict(user_df)[0]

# Display the estimated house price
